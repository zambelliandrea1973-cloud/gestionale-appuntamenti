...
export default function Clients() {
  ...
  // Fetch staff/collaborators for grouping
            ...
            {filteredClients.length === 0 ? (
              ...
              <div className="space-y-6">
                {Object.entries(clientsByStaff).map(([ownerId, staffClients]: [string, any]) => {
                  // Trova il nome del professionista
                  const staffMember = collaborators.find((c: any) => c.id === parseInt(ownerId));
                  const isAdminClients = parseInt(ownerId) === currentUser.id;
                  const staffName = isAdminClients 
                    ? "👑 Admin - Clienti Personali" 
                    : staffMember 
