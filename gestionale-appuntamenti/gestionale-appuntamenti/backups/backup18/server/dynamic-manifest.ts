...
export function serveDynamicManifest(req: Request, res: Response) {
  try {
    ...
    // ID univoco per permettere installazione multipla di PWA (admin + client)
    // Usa URL assoluti per garantire unicità su Android/iOS
    const protocol = req.get('x-forwarded-proto') || req.protocol || 'https';
    const hostname = req.get('host') || 'app.sliplane.io';
    const clientManifestId = ownerUserId 
      ? `${protocol}://${hostname}/client-area?owner=${ownerUserId}` 
      : `${protocol}://${hostname}/client-area`;
