...
export function registerSimpleRoutes(app: Express): Server {
  ...
  // Endpoint per eliminare un cliente - CON PROTEZIONE MULTI-TENANT
  app.delete("/api/clients/:id", (req, res) => {
    ...
    console.log(`🗑️ [DELETE] Tentativo eliminazione cliente ID ${clientId} per utente ${user.id} (${user.type})`);
    ...
    try {
      ...
      if (!clientToDelete) {
        console.log(`❌ [DELETE] Cliente ID ${clientId} non trovato`);
        return res.status(404).json({ message: "Cliente non trovato" });
      }
      
      // PROTEZIONE MULTI-TENANT: Solo owner o admin possono eliminare
      const canDelete = user.type === 'admin' && clientToDelete.ownerId === user.id || 
                       clientToDelete.ownerId === user.id;
      
      if (!canDelete) {
        console.log(`🔒 [DELETE] ACCESSO NEGATO: Utente ${user.id} (${user.type}) ha tentato di eliminare cliente ID ${clientId} di proprietà dell'utente ${clientToDelete.ownerId}`);
        return res.status(403).json({ 
          message: "Non puoi eliminare clienti di altri account",
          details: `Cliente appartiene all'utente ${clientToDelete.ownerId}`
        });
      }
      
      // Procedi con l'eliminazione
      const updatedClients = clients.filter(c => c.id !== clientId);
      storageData.clients = updatedClients;
      
      // Rimuovi anche dai clienti generati di default se presente
      if (storageData.defaultClientsGenerated && storageData.defaultClientsGenerated[clientToDelete.ownerId]) {
        storageData.defaultClientsGenerated[clientToDelete.ownerId] = 
          storageData.defaultClientsGenerated[clientToDelete.ownerId].filter(id => id !== clientId);
      }
      
      // Rimuovi contatori accesso se presenti
      if (storageData.clientAccessCounts && storageData.clientAccessCounts[clientId]) {
        delete storageData.clientAccessCounts[clientId];
      }
      
      saveStorageData(storageData);
      
      const actionType = user.type === 'admin' && clientToDelete.ownerId !== user.id ? 
                        'admin cross-account' : 'owner';
      
      console.log(`✅ [DELETE] Cliente ID ${clientId} "${clientToDelete.firstName} ${clientToDelete.lastName}" eliminato da utente ${user.id} (${actionType})`);
