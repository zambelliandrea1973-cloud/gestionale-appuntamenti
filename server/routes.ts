...
`
  ...
  // Endpoint specifico per il logout dell'area client con pagina di chiusura
  app.get("/api/client-exit", (req: Request, res: Response) => {
    ...
    req.logout((err) => {
      ...
      // Invia una pagina HTML personalizzata che pulisce il localStorage e fornisce un'esperienza di chiusura
      const html = `
      <!DOCTYPE html>
      <html>
      <head>
        ...
        <script>
          // Rimuovi i token di accesso dal localStorage al caricamento della pagina
          window.onload = function() {
            localStorage.removeItem('clientAccessToken');
            localStorage.removeItem('clientId');
            
            // Prova a chiudere automaticamente dopo un secondo (solo se in PWA)
            setTimeout(function() {
              if (window.history && window.history.length > 1) {
                window.history.go(-2); // Torna indietro di due pagine
              }
            }, 1000);
          };
          ...
          function goBack() {
            if (window.history && window.history.length > 1) {
              ...
              window.location.href = "/client-login"; // Fallback
            }
            return false;
          }
        </script>
      ...
      <body>
        <div>
          ...
          <p style="font-size: 0.9em; margin-top: 12px; color: #94a3b8;">Puoi chiudere questa finestra o premere "Torna indietro".</p>
      ...
      // Invia la pagina HTML di chiusura
